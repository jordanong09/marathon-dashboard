...
7. In Campaigns → Overview, press "Import existing KL Half code lists (one-time)". Check both campaigns and their code counts, then delete `promo_lists/` from the repository so real codes no longer live in source control.
...
- Plan Allocation: category capacity and the group allocations, with save history.
...
- Complimentary: programmes, their linked COMPLIMENTARY_ tags, and every issuance.
- Campaigns: campaigns, shared codes and uploaded unique code lists.
